...
class Plot(QDialog):
    ...
    def attribute_selected(self):
        ...
        try:
            ...
            # Remove existing zone-related traces
            js_code = """
    ...
    def update_zone_ticks(self):
        print("[DEBUG] Skipping update_zone_ticks: No valid zone selected.")
        """Adds ticks and forces an immediate re-render."""
        if self.selected_zone is None or self.selected_zone == "Select Zone" or self.selected_zone_df is None:
            print("[DEBUG] Skipping update_zone_ticks: No valid zone selected.")
            return

        try:
            print("[DEBUG] Running update_zone_ticks...")

            # 🚀 JavaScript to clear old traces
            js_clear_code = """
            console.log("[JS] Clearing zone traces...");
            var plotDiv = document.getElementById('plotly-div');
            if (plotDiv) {
                var clearData = plotDiv.data.filter(trace => 
                    !(trace.line && (trace.line.color === 'red' || trace.line.color === 'blue')) &&
                    !(trace.error_y && trace.error_y.color === 'red') &&
                    !(trace.name && trace.name.includes('Zone')) &&
                    !(trace.fill && trace.fill === 'tonexty')
                );
                Plotly.react(plotDiv, clearData, plotDiv.layout);
                console.log("[JS] Successfully cleared traces.");
            } else {
                console.log("[JS] plotly-div not found!");
            }
            """

            def js_callback(result):
                print(f"[DEBUG] JS Callback Result: {result}")

            # 🚀 Run JavaScript and log output
            self.plot_widget.page().runJavaScript(js_clear_code, js_callback)

            # ✅ **Interpolate values for tick positions**
            interpolation_map = {
                'Cumulative Distance': {row['Top_Depth']: self.interpolate_value(row['Top_Depth'], 'Cumulative Distance') 
                                        for _, row in self.selected_zone_df[self.selected_zone_df['UWI'] == self.current_UWI].iterrows()},
                'TVD': {row['Top_Depth']: self.interpolate_value(row['Top_Depth'], 'TVD') 
                        for _, row in self.selected_zone_df[self.selected_zone_df['UWI'] == self.current_UWI].iterrows()}
            }

            # ✅ **Prepare traces for new ticks**
            traces = []
            attribute = self.zone_attribute_selector.currentText()
            color_zones = attribute != "Select Zone Attribute"

            # ✅ **Filter and preprocess zone data**
            zone_data = self.selected_zone_df[
            ...
            # ✅ **Compute color range if needed**
            if color_zones and attribute in zone_data.columns:
                self.min_attr = zone_data[attribute].min()
                self.max_attr = zone_data[attribute].max()
                self.color_colorbar.display_color_range(self.min_attr, self.max_attr)
            else:
            ...
            # ✅ **Batch create traces for each tick**
            for _, zone_row in zone_data.iterrows():
                top_md = zone_row['Top_Depth']
                base_md = zone_row['Base_Depth']

                # ✅ **Use precomputed interpolation values**
                top_cum_dist = interpolation_map['Cumulative Distance'].get(top_md)
                base_cum_dist = interpolation_map['Cumulative Distance'].get(base_md)
                top_tvd = interpolation_map['TVD'].get(top_md)
                base_tvd = interpolation_map['TVD'].get(base_md)

                if not all([top_cum_dist, base_cum_dist, top_tvd, base_tvd]):
                    continue

                # ✅ **If color-based zones are enabled, get colors**
                if color_zones and attribute in zone_row:
                    attribute_value = zone_row[attribute]
                    color = self.color_colorbar.map_value_to_color(
                        attribute_value, 
                        self.min_attr, 
                        self.max_attr, 
                        self.color_colorbar.selected_color_palette
                    )
                    ...
                    traces.append(go.Scatter(
                        ...
                        fill='tonexty',
                        fillcolor=f'rgba({color.red()}, {color.green()}, {color.blue()}, 0.2)',
            ...
            # ✅ **JavaScript to force Plotly redraw**
            js_redraw_code = """
            console.log("[JS] Running Plotly.redraw()...");
            var plotDiv = document.getElementById('plotly-div');
            if (plotDiv) {
                Plotly.redraw(plotDiv);
                console.log("[JS] Plotly.redraw() executed.");
            } else {
                console.log("[JS] plotly-div not found!");
            }
            """
            self.plot_widget.page().runJavaScript(js_redraw_code, js_callback)

            # ✅ **Add new traces and reload Plot**
            if traces:
                self.fig.data = []
                ...
                # ✅ **Force QtWebEngine to repaint instantly**
                self.force_qtwebengine_repaint()
    ...
    def hard_force_qt_refresh(self):
        """🚀 Hard forces a full refresh of QtWebEngineView to ensure UI updates immediately."""
        self.plot_widget.setUpdatesEnabled(False)  # Disable updates
        self.plot_widget.update()  # Trigger repaint
        QApplication.processEvents()  # Force event loop to process
        self.plot_widget.setUpdatesEnabled(True)  # Re-enable updates


    def force_qtwebengine_repaint(self):
        """🚀 Forces QWebEngineView to repaint immediately."""
        print("[DEBUG] Forcing QtWebEngineView repaint...")
    
        self.plot_widget.setUpdatesEnabled(False)  # Disable updates
        self.plot_widget.update()  # Request update
        QApplication.processEvents()  # 🚀 Process Qt events NOW
        self.plot_widget.setUpdatesEnabled(True)  # Re-enable updates

        # 🚀 Absolute last resort: Reload the entire frame
        self.plot_widget.page().runJavaScript("console.log('[JS] Triggering forced UI refresh'); Plotly.redraw();")
    ...
    def clear_zone_traces(self):
        print("Clearing all zone-related data and traces.")

        # First, set the correct div ID in the Plotly HTML
        js_set_div_id = """
        var plotDiv = document.getElementsByClassName('plotly-graph-div')[0];
        if (plotDiv) {
            plotDiv.id = 'plotly-div';
        ...
        # Now, force a full reset and redraw
        js_code = """
        var plotDiv = document.getElementById('plotly-div');
        if (plotDiv) {
            Plotly.newPlot(plotDiv, [], {
                title: 'Seismic & Well Plot',
                xaxis: { title: 'Cumulative Distance' },
                yaxis: { title: 'TVD / Depth' }
            });
        ...
        # Run JavaScript in the correct order
        self.plot_widget.page().runJavaScript(js_set_div_id, lambda _: 
            self.plot_widget.page().runJavaScript(js_code)
        )
    ...
    def zone_selected(self):
        ...
        # If "Select Zone" is chosen, clear everything!
        if self.selected_zone == "Select_Zone":
            ...
            self.color_colorbar.display_color_range(0, 0)  # Reset color range
        
            self.clear_zone_traces()  # Call method to remove ticks & zone traces
        
            js_force_clear = """
            var plotDiv = document.getElementById('plotly-div');
            if (plotDiv) {
                Plotly.react(plotDiv, [], plotDiv.layout);  // Force clear all traces
            }
            """
            self.plot_widget.page().runJavaScript(js_force_clear)
        
            self.zone_attribute_selector.blockSignals(False)
            return  # Stop further execution

        # If a valid zone is selected, fetch data
        try:
    ...
    def finalize_zone_update(self):
        """Handles UI updates after clearing the previous zone data."""
        if self.selected_zone_df is not None:
            self.populate_zone_attribute()
            self.zone_attribute_selector.setEnabled(True)
            self.update_zone_ticks()
    ...
    def update_zone_ticks(self):
        ...
        if (self.selected_zone is None or 
            self.selected_zone == "Select_Zone" or 
            self.selected_zone_df is None):
            return

        # DEBUG: Print data sizes
        print("Zone Data Shape:", self.selected_zone_df.shape)
        print("Current Well Data Shape:", self.current_well_data.shape)
        print("Number of rows for current UWI:", 
              len(self.selected_zone_df[self.selected_zone_df['UWI'] == self.current_UWI]))
        ...
        try:
            ...
            if tick_traces or zone_fills:
                ...
                js_force_redraw = """
                console.log("[JS] Forcing Plotly to re-render...");
                var plotDiv = document.getElementById('plotly-div');
                if (plotDiv) {
                    Plotly.redraw(plotDiv);
                    console.log("[JS] Plotly.redraw() executed.");
                } else {
                    console.error("[JS] plotly-div not found!");
                }
                """
                self.plot_widget.page().runJavaScript(js_force_redraw)
