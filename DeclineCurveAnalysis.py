...
class DeclineCurveAnalysis:
    ...
    def calculate_production_rates(self):
        ...
        return self.prod_rates_all, self.sum_of_errors, self.model_data

    def update_prod_rate(self, updated_model, uwi_combined_df, iterate = False):
        #print(updated_model)
        self.iterate_di = iterate
        self.update = True
        self.uwi_combined_df = uwi_combined_df
        #print("df", self.uwi_combined_df)
        self.uwi_model_data = updated_model[0]
        #print(self.uwi_model_data)
        self.current_uwi = self.uwi_model_data['UWI']
     
        self.production_rates_data = None
        #print(self.sum_of_errors)
  # Assuming updated_model is a list of dictionaries

    ## Assuming there's already a DataFrame where uwi matches this UWI
    #    if 'UWI' in self.uwi_prod_rates_all.columns and self.current_uwi in self.uwi_prod_rates_all['UWI'].values:
    #        self.uwi_combined_df = self.uwi_prod_rates_all[self.uwi_prod_rates_all['UWI'] == self.current_uwi].sort_values(by='date')
    ...
    def iterate_nominal_di(self):
        ...
        if self.iterate_di == False:
            ...
            try:
                                # Set reasonable initial guesses
                initial_guesses_oil = [self.qi_oil, self.nominal_di_oil, self.b_factor_oil]
                bounds_oil = ([self.qi_oil-2000, 1.1 , .05], [self.qi_oil+2000, np.inf, .95])
            ...
            # Fit the model
            try:
                initial_guesses_gas = [self.qi_gas, self.nominal_di_gas, self.b_factor_gas]
            ...
            try:
                ...
                self.di_oil = round((1- ((1 / (self.nominal_di_oil * self.b_factor_oil + 1)) ** (1 / self.b_factor_oil))) * 100,2)
          
                self.nominal_di_oil = (((1 - (self.di_oil / 100)) ** (-self.b_factor_oil)) - 1) / self.b_factor_oil
            ...
            try:
                ...
                self.di_gas = round((1- ((1 / (self.nominal_di_gas * self.b_factor_gas + 1)) ** (1 / self.b_factor_gas))) * 100,2)
                print(self.di_gas)
                self.nominal_di_gas = (((1 - (self.di_gas / 100)) ** (-self.b_factor_gas)) - 1) / self.b_factor_gas 
    ...
    def initialize_oil_parameters(self):
        ...
        if self.iterate_di == False:
            ...
            self.b_factor_oil = float(self.uwi_model_data['b_factor_oil'])
            self.max_oil_date_timestamp = pd.to_datetime(self.uwi_model_data['max_oil_production_date'])
            
            self.nominal_di_oil = (((1 - (self.di_oil / 100)) ** (-self.b_factor_oil)) - 1) / self.b_factor_oil if self.b_factor_oil != 0 else self.di_oil / 100
    ...
    def initialize_gas_parameters(self):
        ...
        if self.iterate_di == False:
            ...
            self.b_factor_gas = float(self.uwi_model_data['b_factor_gas'])
            self.max_gas_date_timestamp = pd.to_datetime(self.uwi_model_data['max_gas_production_date'])
            self.nominal_di_gas = (((1 - (self.di_gas / 100)) ** (-self.b_factor_gas)) - 1) / self.b_factor_gas if self.b_factor_gas != 0 else self.di_gas / 100
    ...
    def _calculate_oil_production(self, row, row_date_timestamp):
        ...
        if self.nominal_di_oil == 0:
            ...
            time_dt_oil = self.nominal_di_oil / (1 + self.b_factor_oil * self.nominal_di_oil * time_oil_years)
        ...
        elif self.b_factor_oil == 0:
            q_oil = self.qi_oil * math.exp(-(self.nominal_di_oil * time_oil_years))
        elif self.b_factor_oil == 1.0:
            q_oil = self.qi_oil / (1 + self.nominal_di_oil * time_oil_years)
        else:
            q_oil = self.qi_oil / (1 + (self.b_factor_oil * self.nominal_di_oil  * time_oil_years))** (1 / self.b_factor_oil) if not np.isnan(time_oil_years) else np.nan
    ...
    def _calculate_gas_production(self, row, row_date_timestamp):
        ...
        if self.nominal_di_gas == 0:
            ...
            time_dt_gas = self.nominal_di_gas / (1 + self.b_factor_gas * self.nominal_di_gas * time_gas_years)
        ...
        elif self.b_factor_gas == 0:
            q_gas = self.qi_gas * math.exp(-(self.nominal_di_gas * time_gas_years))
        elif self.b_factor_gas == 1.0:
            q_gas = self.qi_gas / (1 + self.nominal_di_gas * time_gas_years)
        else:
            q_gas = self.qi_gas / (1 + (self.b_factor_gas * self.nominal_di_gas * time_gas_years)) ** (1 / self.b_factor_gas) if not np.isnan(time_gas_years) else np.nan
    ...
    def forcast_rates(self):
        ...
        while True:
            ...
            new_row = {
                'date': row_date_timestamp,
                'UWI': last_entry['UWI']  # Assuming 'UWI' is available in last_entry
            } # Update the date in the row

            # Calculate oil production rate and error
            q_oil, error_oil = self._calculate_oil_production(new_row, row_date_timestamp)
            oil_revenue = q_oil * net_price_oil
            ...
            gas_revenue = q_gas * net_price_gas
            total_revenue = gas_revenue + oil_revenue
            discounted_revenue = total_revenue * 1/(1+discount_rate/100)
            new_row.update({
